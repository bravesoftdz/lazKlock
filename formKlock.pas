...
begin
  ...
  case PageControl1.TabIndex of
    ...
    begin                    //  event page
      ...
      begin
        stsBrInfo.Panels.Items[3].Text :=
          format('Reminder set for %.2d:%.2d - %s', [SpnEdtHour.Value, SpnEdtMins.Value,
          DatetoStr(DtEdtEvent.Date)]);
...
var
  EvntDt: TDateTime;
begin
  EvntDt := EncodeDateTime(YearOf(DtEdtEvent.Date), MonthOf(DtEdtEvent.Date), DayOf(DtEdtEvent.Date),
    SpnEdtHour.Value, SpnEdtMins.Value, 0, 0);
...
begin
  lblEvent.Caption := format('Event set for %.2d:%.2d - %s', [SpnEdtHour.Value, SpnEdtMins.Value,
    DatetoStr(DtEdtEvent.Date)]);
