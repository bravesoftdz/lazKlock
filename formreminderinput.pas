...
begin
  ...
  str := format('%s, %s, %s, %s, %s', [edtReminderName.Text, DatetoStr(DtEdtReminderDate.Date),
    CmbBxReminderPeriod.Items.Strings[CmbBxReminderPeriod.ItemIndex],
    CmbBxReminderType.Items.Strings[CmbBxReminderType.ItemIndex], BoolToStr(ChckBxReminderActive.Checked)]);
  ...
  begin  //  anniversary file does not exist, create it.
    ...
    try
      ...
      WriteLn(ReminderFile,
        '-------------------------------------------------------------------------------------------------');
      WriteLn(ReminderFile,
        '- A Reminder File for Klock                                                                     -');
      WriteLn(ReminderFile,
        '- Reminder data held in a comma separated text file in the following format                     -');
      WriteLn(ReminderFile,
        '- name, date, period [Yearly/Monthly], type[Wedding/Birthday/Motor/One Off/Other, active[-1/0]] -');
      WriteLn(ReminderFile,
        '-------------------------------------------------------------------------------------------------');
    except
